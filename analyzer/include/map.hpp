...
namespace bridge::analyzer::map
{
    ...
    template <class Iterator, class Output, class Input>
    class map_iterator<Iterator, Output(Input, Input)>
    {
        template <class It, class O, class In>
        friend constexpr bool operator==(
            const map_iterator<It, O(In, In)>&,
            const map_iterator<It, O(In, In)>&
        );

        template <class It, class O, class In>
        friend void swap(
            map_iterator<It, O(In, In)>&,
            map_iterator<It, O(In, In)>&
        );
    public:
        /**
    ...
    private:
        ...
        map_parameters<Output(Input, Input)> m_parameters;

        std::optional<Output> m_cached; ///< The cached last dereferenced value
        Output m_previous;              ///< The previous dereferenced value
    };
    ...
    template <class Iterator, class Output, class Input>
    constexpr bool operator==(
        const map_iterator<Iterator, Output(Input, Input)>& __lhs,
        const map_iterator<Iterator, Output(Input, Input)>& __rhs
    )
    {
        return
            (__lhs.m_inner == __rhs.m_inner) &&
